...
def main(args):
    ...
    return total_reward, weights, bias
...
if __name__ == "__main__":
    ...
    total_reward, weights, bias = main(parser.parse_args(['mc', 'raw', 'mc_raw_weight.out', 'mc_raw_returns.out',
                            '2000', '200', '0.05', '0.999', '0.001']))
    total_reward1, weights1, bias1 = main(parser.parse_args(['mc', 'tile', 'mc_tile_weight.out', 'mc_tile_returns.out',
                            '400', '200', '0.05', '0.99', '0.00005']))

    plot1 = plt.figure(1)
    roll = [np.mean(total_reward[i: i+25]) for i in range(2000-25)]
    blue_line = plt.plot(list(range(1, 2001)), total_reward, color='blue', label='per episode')
    red_line = plt.plot(list(range(1, 2000-25 + 1)), roll, color='red', label='rolling mean over a 25 episode window')
    plt.xlabel('number of episode')
    plt.ylabel('return(sum of all rewards in an episode')
    plt.xlim([1, 2000])
    plt.title('Return vs episode of Raw Feature on the Mountain car')
    plt.legend()
    plt.show()

    plot2 = plt.figure(2)
    roll = [np.mean(total_reward1[i: i + 25]) for i in range(400 - 25)]
    blue_line = plt.plot(list(range(1, 401)), total_reward1, color='blue', label='per episode')
    red_line = plt.plot(list(range(1, 400-25+1)), roll, color='red', label='rolling mean over a 25 episode window')
    plt.xlabel('number of episode')
    plt.ylabel('return(sum of all rewards in an episode')
    plt.xlim([1,400])
    plt.title('Return vs episode of Tile Feature on the Mountain car')
    plt.legend()
    plt.show()
