...
def test(X, X1, nn):
    ...
    return y, y1, y_tr_label, y_te_label, error_tr, error_te
    raise NotImplementedError
...
train_input = 'small_train_data.csv'
validation_input = 'small_validation_data.csv'
...
for index, hids in enumerate(n_hid_array):
    n_hid = hids
    # my_nn = NN()
    if init_flag == 1:
        #len(set(y_tr))
    ...
    y_tr_pred, y_te_pred, y_tr_label, y_te_label,  error_tr, error_te = test(X_tr, X_te, my_nn)
    cross_tr.append(-np.average(np.diag(np.dot(np.log(y_tr_pred), np.transpose(y_tr_mod)))))
    cross_te.append(-np.average(np.diag(np.dot(np.log(y_te_pred), np.transpose(y_te_mod)))))

plot1 =plt.figure(1)
blue_line = plt.plot(n_hid_array, cross_tr, color='blue',marker ='o',  label='Train Data')
red_line = plt.plot(n_hid_array, cross_te, color='red',marker ='o',  label='Validation Data')
plt.xlabel('number of hidden units')
plt.ylabel('average cross-entropy')
plt.title('Average Train and Validation Cross-Entropy Loss')
plt.xlim([0, 200])
plt.legend()
plt.show()
...
file = open("metrics_sgd_small.txt", "r")
lines = file.readlines()[1::2]
sgd = []
for i in range(len(lines)-1):
    sgd.append(float(lines[i].split()[-1]))

epoch = list(range(1,n_epochs+1))
plot2 = plt.figure(2)
#ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
blue_line = plt.plot(epoch, sgd, color='blue',  label='SGD')
red_line = plt.plot(epoch, JDt, color='red', label='SGD with Adagrad')
plt.xlabel('number of epochs')
plt.ylabel('average cross-entropy')
plt.title('Average Validation Cross-Entropy Loss of SGD with and without AdaGrad')
plt.xlim([0, 100])
#plt.ylim([1, 3])
plt.legend()
plt.show()
...
n_hid_array = 50
init_flag = 1
lr_array = [0.1, 0.01, 0.001]


for index, lrs in enumerate(lr_array):
    lr = lrs
    # my_nn = NN()
    if init_flag == 1:
        #len(set(y_tr))
    ...
    y_tr_pred, y_te_pred, y_tr_label, y_te_label,  error_tr, error_te = test(X_tr, X_te, my_nn)
    epoch = list(range(1, n_epochs + 1))
    plot =plt.figure(3+index)
    blue_line = plt.plot(epoch, JD, color='blue',  label='Train Data')
    red_line = plt.plot(epoch, JDt, color='red',  label='Validation Data')
    plt.xlabel('number of epochs')
    plt.ylabel('average cross-entropy')
    plt.title('Average Cross-Entropy Loss with learning rate = '+ str(lr))
    plt.xlim([0, 100])
    plt.legend()
    plt.show()
